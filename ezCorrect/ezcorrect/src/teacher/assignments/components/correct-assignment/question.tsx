...
const Question: React.FC<IQuestionProps> = (props) => {
  ...
  return (
    <Card
      ...
      style={{
        backgroundColor:
          props.question.id === selectedQuestion.id
            ? darken(colorRenderer(props.question.status), 0.4)
            : colorRenderer(props.question.status),
      }}
    ...
    >
      <Grid container alignItems="center" justify="space-between">
        ...
        <Grid item>
          <h5 className={classes.points}>
            ( {renderPoints()} / {renderMaxPoint()} poäng )
          </h5>
        </Grid>
      </Grid>
